...
void assign_pipe_fds(t_minishell ms, t_pipe_data *pdata, int *redir_fd, int idx)
{
	int new_pipe[2];
...
/// @param pdata Struct used for the execution of pipes
void child_parse_and_exe(t_minishell ms, t_tree_node *node, t_pipe_data *pdata)
{
	// cmd parse --
	if (node->right)
	...
	if (execve(pdata->path, pdata->cmd, pdata->env) == -1)
		minishell_clean(ms, error_code_for_exec(pdata)); // fail execve ABORT
}
...
{
	if (access(pdata->path, F_OK) < 0)
	{
		// make an error message function
		return(127);
	}
	else if (access(pdata->path, X_OK) < 0)
	{
		// make an error message function
		return(126);
	}
